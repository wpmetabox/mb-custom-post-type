...
class PostTypeDefinitionAbilities {

	private const POST_TYPE = 'mb-post-type';
	private const SLUG      = 'post-type';
	private const LABEL     = 'post type';
	...
	private function register_get(): void {
		wp_register_ability(
			'meta-box/get-' . self::SLUG . 's',
			[
				'label'               => sprintf( __( 'Get %ss', 'mb-custom-post-type' ), self::LABEL ),
				'description'         => sprintf( __( 'List or read %s definitions.', 'mb-custom-post-type' ), self::LABEL ),
				'category'            => 'meta-box',
				...
				'input_schema'        => [
					...
					'properties' => array_merge(
						...
						[
							'settings' => [
								'type'        => 'object',
								'description' => sprintf( __( 'The %s settings object (slug, labels, supports, etc.).', 'mb-custom-post-type' ), self::LABEL ),
							],
