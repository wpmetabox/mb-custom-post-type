		...
		<div className="ptg-field">
			...
			<div className="ptg-input">
				<ul className="ptg-input-list">
					{values.map( (value, key) => <li key={key}><label className="ptg-description"><input type="checkbox" name={value.name} defaultChecked={value.checked} onChange={update} /> {value.description}</label></li> )}
				</ul>
			</div>
		</div>
	)
